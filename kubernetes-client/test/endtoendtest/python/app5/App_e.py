...
talk_with_a = "10.96.0.11:8090/e"
talk_with_b = "10.96.0.12:8091/e"
talk_with_c = "10.96.0.13:8092/e"
talk_with_d = "10.96.0.14:8093/e"
talk_with_e = "10.96.0.15:8094/e"


def get_messge(length):

    return [0] * length

def send_message(talk_with, size):

    sys.stdout.write("Started requests form E to " +talk_with+ '\n')
...
class KodeFunHTTPRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path == '/a':
            ...
            send_message(talk_with_a,3000)
            sys.stdout.write("TEST A->A" + '\n')
            return 

        if self.path == '/b':
             
            send_message(talk_with_b,3000)
            sys.stdout.write("TEST A->B" + '\n')
            return 

        if self.path == '/c':

            self.send_response_t(9000000)
        ...
        if self.path == '/d':
            ...
            sys.stdout.write("Response D<-E" + '\n')
            return

        if self.path == '/e':
            
            send_message(talk_with_e,1000)
