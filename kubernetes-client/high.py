...
def calculate_distance(u, v, w=None):
     print("calculate distance",Dist[int(u[0])][int(v[0])])
...
#display_linkage_matrix(linked)

#Create dict
json_file = {}

sortedClusters = collections.OrderedDict(sorted(extract_levels(linked,X).items()))

for key in range(num_services):
	sortedClusters[key] = []
	sortedClusters[key].append(key)
        

json_file['service_clusters'] = sortedClusters
